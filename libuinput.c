...
struct uinput_tkn *uinput_open(char *path) {
	...
	tkn = (uinput_token*)malloc(sizeof(struct uinput_token));
	if (!tkn)
		return NULL ;
...
void uinput_close(struct uinput_tkn *tkn) {
	struct uinput_token *t = (uinput_token*) tkn;
 ...
 */
int uinput_set_valid_events(struct uinput_tkn *tkn, struct input_events_custom *ev,
		unsigned int n) {
	int i, err = 0;
